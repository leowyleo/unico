...
![Unico 重复组、缩略图与文件预览](docs/promo/screenshots/results-zh.jpeg)

*真实 App 截图，使用生成的演示文件，不含个人资料。*
...
- **自动建议保留一份。** 默认保留创建时间最早的文件，也可换一份保留，或取消某个副本的清理。“推荐保留”不等于认定它是原件。
- **只移到废纸篓。** 清理前再次核验文件身份和内容，每组至少保留一份，不提供永久删除或清空废纸篓。
- **克制的原生界面。** 默认英文，标题栏地球图标可即时切换简体中文；外观跟随 macOS 深浅色。

## 两种扫描范围

**全盘扫描**默认跳过隐藏目录、系统、应用内部、已识别的开发项目、图库及未知类型，不自动包含外接与网络磁盘。

**用户主动选择的文件夹优先于默认排除规则，但开始前必须确认风险。** 确认后，所选目录及子目录里的隐藏文件、应用与项目资源、可执行文件和未知类型都会参与比较。授权只对本次目录范围生效，不会影响全盘扫描。
